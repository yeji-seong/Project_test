...
Test <- function(x_new, x_old, y){
  ...
  returnValue((SSR_new - SSR_old)*(n_new-p_new-1)/SSE_new)
}
```
...
F <- Test(X_new, X_old, y)
qf(0.95, 1, 17)

# F = 39.00452 > Fin = F(1, 17, 0.05) = 4.451322
# x3 변수로 선택
```
...
# F = 6.664173 > Fin = F(1, 16, 0.05) = 4.493998
# x5 변수로 선택
...
# F = 1.487634 > Fin = F(1, 15, 0.05) = 4.543077
# x2 변수로 선택하지 않음
# 변수선택 종료
```
...
# Adjusted R에서 F검정 결과 x2는 변수로 선택하지 않음
# 변수선택 종료
```
#### model : y = b0 + b3*x3 + b5*x5 + e
...
# F = 0.3060349 <= Fin = F(1, 15, 0.05) = 4.543077
# x4 변수로 선택하지 않음
# 변수선택 종료
```
#### model : y = b0 + b3*x3 + b5*x5 + e
